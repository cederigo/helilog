...
- **THEN** all relevant files (types, errors, repository interface, service, Prisma impl, schema, routes) SHALL be found within `apps/backend/src/helicopter/`
...
- **THEN** `apps/backend/src/routes/` SHALL no longer exist and SHALL be replaced by per-domain route files

### Requirement: Shared module for cross-cutting types

The `src/shared/` module SHALL provide a base `AppError` class and shared primitive types (e.g. `PaginationParams`, `PaginatedResult`) used across domain modules. Types that are also consumed by the frontend SHALL additionally be exported from `packages/shared` as `@helilog/shared`.
